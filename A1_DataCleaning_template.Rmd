...
# git remote set-url origin https://github.com/victor-m-p/project
...
#demo_train: Visit, Child.ID 
#LU_train: VISIT, SUBJ
#token_train: VISIT, SUBJ

#changing the two that are alike and merging. 
speech <- merge(LU_train, token_train, all = T) %>%
  rename(Visit = VISIT, Child.ID = SUBJ)

#not needed anymore.
rm(LU_train, token_train)
...
compare_df_cols(speech, demo_train)
head(demo_train$Visit)
head(speech$Visit)
...
df <- df %>%
  mutate_at("Child.ID", str_replace_all, "\\.", "")
}

demo_train <- remover(demo_train)
speech <- remover(speech)

str(demo_train$Child.ID)
str(speech$Child.ID)
...
demo_selected <- selector(demo_train)
speech_selected <- selector(speech)
...
#now it works (probably the as.integer stuff.)
data_merged <- merge(demo_selected, speech_selected, all = T)
