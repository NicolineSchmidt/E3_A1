...
## remembering tidyverse. 
# filter, arrange, select, mutate, summarise, slice, group_by. 
...
str(demo_train$VISIT)
str(LU_train$VISIT)
str(token_train$VISIT)

#we want "1, 2, 3, 4, 5, 6" but as_factor (I think). 
...
#Solution 1: (pretty dumb, and gets us only 352 obs.)
merged_data <- merge(demo_train, LU_train,
                     by = c("SUBJ", "VISIT"))

merged_data <- merge(merged_data, token_train, 
                     by = c("SUBJ", "VISIT"))
...
new_data <- merged_data %>%
  filter(VISIT == 1) %>%
  select(SUBJ, ADOS, MullenRaw, 
         ExpressiveLangRaw, Socialization)

#renaming clinical variables (but in which data-frame?)
